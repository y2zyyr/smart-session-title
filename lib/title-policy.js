 ...
 * ------------------------------------------------------------------ */
/**
 * One exclusion term, compiled once per generation.
 *
 * Matching is deliberately literal — no stemming, no alias resolution, no
 * translation. A term containing ASCII letters is matched case-insensitively
 * ("Acme" hides "ACME"); every other term is matched exactly, which is what
 * "原文匹配" means for Chinese. Users add each variant they care about.
 */
function compileExclusion(term) {
    return {
        term,
        // Escaped for a regular expression, so a term like "a.b" or "(内部)" is
        // matched literally instead of becoming a pattern.
        source: term.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"),
        flags: /[A-Za-z]/u.test(term) ? "giu" : "gu"
    };
}
/**
 * Compile the configured exclusion list, dropping anything unusable.
 *
 * Settings validation already guarantees non-empty single-line strings; this is
 * defensive so a hand-built policy (tests, or a future caller) cannot make the
 * redaction throw in the middle of a generation.
 *
 * @param exclusions - the raw list, possibly undefined.
 * @returns one entry per usable term, in the user's order.
 */
export function compileTitleExclusions(exclusions) {
    ...
    for (const term of exclusions) {
        if (typeof term !== "string" || term.length === 0)
            continue;
        out.push(compileExclusion(term));
    }
    return out;
}
function regexOf(compiled) {
    return new RegExp(compiled.source, compiled.flags);
}
/**
 * The first configured term that still occurs in `text`, or undefined.
 *
 * Used as the post-generation gate: the check runs on the exact string that is
 * about to be stored, so a term cannot slip through by appearing only after
 * shortening, and a term cut in half by shortening is not falsely reported.
 *
 * @param text - the candidate title (post-validation, post-truncation).
 * @param compiled - output of {@link compileTitleExclusions}.
 * @returns the offending term, for a diagnostic that never logs the title.
 */
export function findExcludedTerm(text, compiled) {
    for (const entry of compiled) {
        if (regexOf(entry).test(text))
            return entry.term;
    }
    return undefined;
}
 ...
 *
 * This is the pre-generation treatment (the model never sees the term) and the
 * deterministic last resort when one survives into the model's own output. It is
 * DELETION rather than substitution on purpose: the requested behaviour for
 * "修复客户甲的订单导出错误" is "修复订单导出错误", and inventing a replacement
 * name would be a second guess about content we are trying to remove.
 *
 * Line structure is preserved (the caller may still run structural extraction
 * over the result); only the whitespace left behind on an affected line is
 * repaired, so removing a word mid-sentence cannot leave a double space.
 *
 * @param text - the text to redact.
 * @param compiled - output of {@link compileTitleExclusions}.
 * @returns the redacted text and how many occurrences were removed.
 */
export function redactExcludedTerms(text, compiled) {
    ...
    const redacted = lines.map((line) => {
        let out = line;
        for (const entry of compiled) {
            const matches = out.match(regexOf(entry));
            if (matches === null)
                continue;
            removed += matches.length;
            out = out.replace(regexOf(entry), "");
        }
        if (out === line)
            return line;
        return out
 ...
 *
 * Order of operations: redact the configured exclusion terms, window the raw
 * text if it is absurdly large, drop large code blocks, then fall back to
 * structural extraction, then to head+tail. The function never rejects an input
 * on size alone.
 *
 * Redaction runs first so that everything downstream — the fence scanner, the
 * structural extractor, the byte accounting — sees only the text that is
 * actually sent. A term removed late could otherwise survive through a window
 * boundary or a kept section.
 *
 ...
 */
export function prepareTitleInput(input, config, exclusions) {
    const rawBytes = byteLength(input);
    const redacted = redactExcludedTerms(input, compileTitleExclusions(exclusions));
    const budget = config.targetPreparedInputBytes;
    let work = redacted.text;
    let rawWindowed = false;
    if (byteLength(work) > config.maxRawInputBytes) {
        work = headTailTrim(work, config.maxRawInputBytes);
        rawWindowed = true;
    }
    ...
    const base = {
        rawBytes,
        excludedTerms: redacted.removed,
