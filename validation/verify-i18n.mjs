...
// SessionId: real settings validation, copy outcomes, and live visibility.
assert(settingsModule.resolveTitleSettings().showSessionId === true, "ID defaults on");
assert(settingsModule.resolveTitleSettings({ showSessionId: false }).showSessionId === false, "ID can be disabled");
let invalidIdSetting = false;
try { settingsModule.resolveTitleSettings({ showSessionId: "false" }); } catch { invalidIdSetting = true; }
assert(invalidIdSetting, "non-boolean ID preference rejected");
fakeScopeSnapshot = { status: "ready", value: { enabled: false }, user: {}, writable: true, revision: 1 };
const idProps = { sessionId: "session-full-id-12345678901234567890", scope, t: tZh };
let copiedId;
window.navigator = { clipboard: { writeText: async value => { copiedId = value; } } };
let idTree = await settle(capturedExport.SessionIdAction, idProps);
let idButton = findFirstElement(idTree, n => n.type === "button");
assert(idButton, "ID remains available with AI disabled");
let stoppedCopy = false;
await idButton.props.onClick({ stopPropagation() { stoppedCopy = true; } });
idTree = renderPass(capturedExport.SessionIdAction, idProps);
assert(copiedId === idProps.sessionId && stoppedCopy, "copies full ID without navigation");
assert(renderTree(idTree).join(" ").includes(tZh("sessionId.copied")), "copy success feedback");
window.navigator.clipboard.writeText = async () => { throw new Error("denied"); };
await findFirstElement(idTree, n => n.type === "button").props.onClick({ stopPropagation() {} });
idTree = renderPass(capturedExport.SessionIdAction, idProps);
assert(findFirstElement(idTree, n => n.type === "input").props.value === idProps.sessionId, "failed copy offers full manual value");
const idSettings = await settle(capturedExport.SettingsSection, { scope, t: tZh });
const idToggle = findFirstElement(idSettings, n => n.props?.id === "sst-showSessionId");
