...
本版重新组织设置页：主要设置直接展示，高级参数折叠；保存状态实时反馈。批量优化通过独立工作区进入，自动加载会话，支持标题或 ID 搜索与返回设置。批量路由显示已保存配置；固定模型模式不再提供重复的自动兜底选项。日期格式仅在添加日期时显示。
...
所有设置先编辑，再点击模型选择行最右侧的「保存设定」统一生效；保存失败保留填写内容。批量优化使用最后保存的模型配置。模型提示与标题操作说明位于页面底部的「使用说明」。
...
「标题格式与内容」展开后，风格、语言和字数同组，日期独立成行，排除词使用整行输入框。
详细规则收在折叠说明中，避免长文案挤占控件空间。

### 复制当前 SessionId

对话顶部以浅灰色小字号等宽字体显示当前会话 ID。点击复制完整 ID（不受显示省略影响），悬停可查看完整值。粘贴到其他对话可帮助 agent 定位此会话；读取需要相应工具与权限。复制其他会话的 ID 需先打开该会话。剪贴板写入失败时提供只读输入框，方便手动复制。

在插件设置中使用 **在对话顶部显示 SessionId** 开关，即时显示或隐藏。默认开启，通过 `showSessionId` 持久保存，独立于 AI 标题生成和标题锁定，不修改会话标题。

在 DSH Desktop 2.0.9 中，ID 单独显示在标题工具栏下方、“对话 / 轨迹”标签上方，并与标签左对齐。此布局使用已核实的 host CSS 类；升级后若类名变化，将回退为标题栏内显示。

会话顶部的「重新生成标题」会按当前标题设置替换现有标题（包括人工标题），不会重新运行对话。「锁定标题」阻止本插件自动、手动和批量生成标题；锁定后显示「标题已锁定」，点击可解锁。锁定不影响聊天、人工改名或 DSH 兜底标题。设置页的「会话顶部的标题操作」中也提供这些说明。

设置页采用紧凑的左右设置行；模型选择行最右侧提供统一保存按钮，标题操作说明默认折叠，批量优化保留独立入口。
